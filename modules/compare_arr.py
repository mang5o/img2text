...
def text_compare(text_array, img_array, text_width, text_height, img_w, img_h):
    ...
    for y_grid in range(h_grid_cnt):
        ...
        for x_grid in range(w_grid_cnt):
            now_loss = 0.0
            for x in range(x_grid*text_width, (x_grid+1)*text_width):
                for y in range(y_grid * text_height, (y_grid + 1) * text_height):
                    now_loss += ov_array[y][x]
            tmp_x_line.append(now_loss)
